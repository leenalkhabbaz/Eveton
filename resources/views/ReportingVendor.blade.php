            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                An Admin block Vendor or ignore Reporting after read and cheak it.
                            </div>
                        </div>
                    </div>
                    <!----->
                                ...
                                <div class="row">
                                    <div class="col-md-20">
                                        <div class="profile-blog blog-border">
                                            <img class="rounded-x" src="{{asset('storage/UserPhoto/VendorProfile/'.$report->reportVendor->profile_thumbnail )}}" alt="">
                                            <div class="name-location">
                                                <strong style="font-size:18px ;margin-top:5%"> UserName: {{ $report->reportVendor->name }}</strong>
                                            </div>
                                            ...
                                            <ul class="list-inline share-list">
                                                ...
                                                <form  method="post" action="/vendor/{{$report->id}}/{{$report->vendor_id}}" id="forbiddin{{$report->vendor_id}}">
                                                    @csrf
                                                    @method('PUT')
                                                    <li>  <button type="submit" class="btn btn-danger" onclick="document.getElementById('forbiddin{{$report->vendor_id}}').submit(); return false;">Block</button> </li>
                                                </form>


                                                <form  method="post" action="/report/{{$report->id}}/ignor" id="ignor{{$report->id}}">
                                                    @csrf
                                                    @method("DELETE")
                                                    <li> <button type="submit" class="btn btn-secondary" onclick="document.getElementById('ignor{{$report->id}}').submit(); return false;"style="margin-top: 10%">Ignore</button> </li>
                                                </form>


                                                {{-- <li> <button type="button" class="btn btn-secondary"> Ignore</button></li> --}}
