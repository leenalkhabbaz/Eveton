            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                An Admin block Hall or ignore Reporting after read and cheak it.
                            </div>
                        </div>
                    </div>
                    <!----->
                                ...
                                <div class="row">
                                    <div class="col-md-20">
                                        <div class="profile-blog blog-border">
                                            <img class="rounded-x" src="{{asset('storage/UserPhoto/VendorProfile/'.$report->reportHall->profile_thumbnail )}}" alt="">
                                            <div class="name-location">
                                                <strong style="font-size:18px ;margin-top:5%"> UserName: {{ $report->reportHall->name }}</strong>
                                            </div>
                                            ...
                                            <ul class="list-inline share-list">
                                                ...
                                                <form  method="post" action="/hall/{{$report->id}}/{{$report->hall_id}}" id="hall{{$report->hall_id}}">
                                                    @csrf
                                                    @method('PUT')
                                                    <li>  <button type="submit" class="btn btn-danger" onclick="document.getElementById('hall{{$report->hall_id}}').submit(); return false;">Block</button> </li>
                                                </form>

                                                <form  method="post" action="/reportHall/{{$report->id}}/ignorHall" id="ignorHall{{$report->id}}">
                                                    @csrf
                                                    @method("DELETE")
                                                    <li> <button type="submit" class="btn btn-secondary" onclick="document.getElementById('ignorHall{{$report->id}}').submit(); return false;"style="margin-top: 10%">Ignore</button> </li>
                                                </form>
